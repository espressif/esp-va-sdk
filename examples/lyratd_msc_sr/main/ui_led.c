...
#endif
    if (xQueueSend(led_st.ui_led_queue, &va_state, 0) != pdTRUE) {
        ESP_LOGE(UI_LED_TAG, "Cannot send assistant state");
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
static void ui_led_task(void *arg)
{
    int va_state;
    while (1) {
        if (xQueueReceive(led_st.ui_led_queue, &va_state, portMAX_DELAY) != pdTRUE) {
