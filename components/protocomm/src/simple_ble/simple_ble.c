...
uint8_t simple_ble_get_uuid(uint16_t handle)
{
    for (int i = 0; i < SIMPLE_BLE_MAX_GATT_TABLE_SIZE; i++) {
        if (g_gatt_table_map[i] == handle) {
            return *g_ble_cfg_p->gatt_db[i].att_desc.uuid_p;
        }
    }
    return -1;
}
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t p_gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
        if (param->add_attr_tab.status != ESP_GATT_OK) {
            ...
            ESP_LOGD(TAG, "created attribute table successfully, the number handle = %d\n", param->add_attr_tab.num_handle);
...
{
    g_ble_cfg_p = cfg;
    ESP_LOGD(TAG, "Free mem at start of simple_ble_init %d\n", esp_get_free_heap_size());
    esp_err_t ret;
    ...
    ESP_LOGD(TAG, "Free mem at end of simple_ble_init %d\n", esp_get_free_heap_size());
    return ESP_OK;
}
...
{
    esp_err_t err;
    ESP_LOGD(TAG, "Free mem at start of simple_ble_stop %d\n", esp_get_free_heap_size());
    ...
    ESP_LOGD(TAG, "Free mem at end of simple_ble_stop %d\n", esp_get_free_heap_size());
    return ESP_OK;
}
