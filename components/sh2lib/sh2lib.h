...
/**
 * @brief Callback function will be invoked when GOAWAY frame is received
 *
 * @param[in] hd        Pointer to a variable of the type 'struct sh2lib_handle'.
 */
typedef void (*sh2lib_on_goaway_receive_callback)(struct sh2lib_handle *handle);
...
struct sh2lib_handle {
    ...
    sh2lib_on_goaway_receive_callback go_away_cb;
};
